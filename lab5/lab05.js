...
if (url.value.indexOf("name=")===-1){
    ...
    url_result.value=url.value.substring(url.value.indexOf("=")+1,url.value.indexOf("&"));
}
}
...
function arrSameStr(){
    var o = {};
    for (var i = 0, length = most.value.length; i < length; i++) {
        var char = most.value.charAt(i);
        if (o[char]) {
            o[char]++;
        } else {
            o[char] = 1;
    ...
    for (var key in o) {
        if (o[key] == max) {
            result.value="The most character is:"+key+" times:"+max;
        }
    }
}
