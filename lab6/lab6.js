...
function testTime(){
    ...
    const interval = window.setInterval(function () {
        ...
        console.log(num);
        i++;
        if (new Date().getSeconds() >= 55 || i >= 10) {
...
function testMail(telephone,mail) {
    ...
    if (!reg.test(telephone)&&!regMail.test(mail)){
    ...
    for (i=0 ; i<array.length-1; i++){
        if (array[i].length===array[i+1].length){
            if (array[i].toUpperCase().search(array[i+1].toUpperCase())===0){
                s.add(array[i]+" "+array[i+1])
            }
        }
    }
    if(s.size>10){
        var s3=new Set();
        var arrSort = Array.from(s).sort(function(s, t) {
...
function testKeyBoard(wantInput, actualInput) {
    var array=wantInput.split("");
    var s=new Set(array);
    var intersect=new Set();
    var array2=actualInput.split("");
    var s2=new Set(array2);
    for (var i of s) {
            if (!s2.has(i)) {
                intersect.add(i.toUpperCase());
            }
        }
...
function twoSum(nums, target) {
    ...
    for (let i = 0; i <nums.length; i++) {
        if (mayMap.has(target - nums[i])) {
            console.log([mayMap.get(target - nums[i]), i]) ;
        }
        mayMap.set(nums[i], i);
    }
}
...
function Country() {
    ...
    if (typeof this.saySad != "function"){
        Country.prototype.saySad=function () {
...
var instance=new PoorCountry();
DevelopedCountry.prototype=Object.create(Country.prototype);
var instance2=new DevelopedCountry();
var instance3=new DevelopingCountry();
